...
* Complete executable reproducer: [reproducer/](https://github.com/WilliePim/camunda7-portability-inventory/tree/main/reproducer), a Maven project with an embedded engine on in-memory H2. In that folder, with `JAVA_HOME` pointing to a JDK 21, run `./mvnw test -Dtest=Bug03ProcessDefinitionKeyMetaTest`.
